...
const GROQ_MODELS = ["llama3-8b-8192", "llama3-70b-8192", "mixtral-8x7b-32768", "gemma-7b-it", "gemma2-9b-it"];

export const ensureGroqModel = (model: string): string => {
    if (GROQ_MODELS.includes(model)) return model;
    if (model.toLowerCase().includes("gemini")) return "llama3-8b-8192";
    return model;
};
...
export const fetchChapters = async (
  ...
  try {
      ...
      if (s) { const p = JSON.parse(s); if(p.aiModel) modelName = ensureGroqModel(p.aiModel); }
  } catch(e){}
...
export const fetchLessonContent = async (
  ...
  try {
      ...
      if (stored) {
          ...
          if (s.aiModel) modelName = ensureGroqModel(s.aiModel); // Allow override, but default is Llama3
...
export const generateCustomNotes = async (userTopic: string, adminPrompt: string, modelName: string = "llama3-8b-8192"): Promise<string> => {
    const safeModel = ensureGroqModel(modelName);
    ...
    return await executeWithRotation(async () => {
        return await callGroqApi([{ role: "user", content: prompt }], safeModel);
    }, 'STUDENT');
};
...
export const generateUltraAnalysis = async (
    ...
    if (settings) {
        if (settings.aiModel) modelName = ensureGroqModel(settings.aiModel);
